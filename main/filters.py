...
from .models import Tool, Rental, Review, Transaction, Dispute, Maintenance, PromotionalCampaign
...
class ToolFilter(filters.FilterSet):
    ...
    def filter_availability(self, queryset, name, value):
        if name == 'available_from':
            return queryset.exclude(rental__start_date__lte=value, rental__end_date__gte=value)
...
class RentalFilter(filters.FilterSet):
    ...
    created_after = filters.DateTimeFilter(field_name='created_at', lookup_expr='gte')
    has_review = filters.BooleanFilter(field_name='review', lookup_expr='isnull', exclude=True)
    has_insurance = filters.BooleanFilter(field_name='insurance', lookup_expr='isnull', exclude=True)
...
class ReviewFilter(filters.FilterSet):
    ...
    tool_name = filters.CharFilter(field_name='rental__tool__name', lookup_expr='icontains')
