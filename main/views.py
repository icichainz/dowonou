...
from .models import Tool, Rental, Review, User, Category
from .forms import ToolForm, RentalRequestForm, ReviewForm, UserPreferencesForm
...
def search_tools(request):
    ...
    tools = Tool.objects.filter(Q(name__icontains=query) | Q(description__icontains=query))
    data = [{'id': tool.id, 'name': tool.name, 'hourly_rate': str(tool.hourly_rate)} for tool in tools]
