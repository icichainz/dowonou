...
class ToolForm(forms.ModelForm):
    ...
    class Meta:
        model = Tool
        fields = ['name', 'description', 'category', 'hourly_rate', 'daily_rate', 'weekly_rate', 'condition', 'location']
        widgets = {
    ...
    def clean(self):
        ...
        if daily_rate and hourly_rate and daily_rate < hourly_rate * 8:
            raise ValidationError("Daily rate should be at least 8 times the hourly rate.")
        if weekly_rate and daily_rate and weekly_rate < daily_rate * 5:
            raise ValidationError("Weekly rate should be at least 5 times the daily rate.")
...
class SearchForm(forms.Form):
    ...
    available_from = forms.DateTimeField(required=False, widget=forms.DateTimeInput(attrs={'type': 'datetime-local'}))
    available_to = forms.DateTimeField(required=False, widget=forms.DateTimeInput(attrs={'type': 'datetime-local'}))
    ...
    def clean(self):
        ...
        if available_from and available_to and available_from > available_to:
            raise ValidationError("'Available from' should be earlier than 'Available to'.")
...
class InsuranceForm(forms.Form):
    ...
    def clean(self):
        ...
        if accept_insurance and not coverage_amount:
            raise ValidationError("Please specify the coverage amount if you want insurance.")
