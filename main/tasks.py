...
def send_rental_reminder():
    ...
    upcoming_rentals = Rental.objects.filter(start_date__date=tomorrow.date(), status='approved')
...
def update_tool_availability():
    ...
    Rental.objects.filter(
        ...
        status='approved'
    ).update(tool__availability=True)
...
def calculate_earnings():
    ...
    """
    completed_rentals = Rental.objects.filter(status='completed', earnings_processed=False)
    ...
    logger.info(f"Calculated earnings for {completed_rentals.count()} completed rentals.")
...
def update_tool_rankings():
    ...
    tools = Tool.objects.annotate(
        ...
        avg_rating=Avg('rental__review__rating')
    )
...
def generate_monthly_report():
    ...
    total_rentals = Rental.objects.filter(created_at__gte=timezone.now() - timezone.timedelta(days=30)).count()
    total_revenue = Rental.objects.filter(status='completed').aggregate(Sum('total_cost'))['total_cost__sum'] or 0
    new_users = User.objects.filter(date_joined__gte=timezone.now() - timezone.timedelta(days=30)).count()
